    ...
    for i = 2:height(result)
        if helpArray(i) > result.TM1_1(i) %  condition to use gas holder
            if result.usage(i-1) == 2 % remain if gas holder is already in use, continue using
                result.usage(i) = 2;
                result.TM1_2(i) = result.gasholder_profit(i) * result.abs_heatdemand(i);
            elseif result.cum_loss2(i) - result.cum_loss1(i) > input.plant.startup % Entry Condition: if gas holder is not in use, profit by using gas holder mut be higher then startup costs  
                result.usage(i) = 2; 
                result.TM1_2(i) = result.gasholder_profit(i) * result.abs_heatdemand(i);
            end
        elseif result.usage(i-1) == 2 & result.cum_loss2(i) - result.cum_loss1(i) > input.plant.startup % remain condition if gas holder TM is lesser then chp TM  
            result.usage(i) = 2;
        end
    end
end
...
if input.storage.enable == 1 
    ...
    for i = 2:height(result)
        ...
        if result.storagelevel(i-1) * input.storage.capacity >= result.newdemand(i) * result.phasepower_th(i) & ... % storage must be able to fullfill demand to 100 %  
           (result.usage(i) == 2 | ...
           (result.cum_loss3(i) - result.cum_loss1(i) > input.plant.startup & result.TM1_1(i) < 0))  % (result.storagelevel(i) * input.storage.capacity * input.market.heatprice) - result.cum_loss2(i) > input.plant.startup) 
 ...
 %% Anteil ungekpoppelte Erzeugung 
 result.lostratio = result.lostheat ./ result.abs_heatdemand;
